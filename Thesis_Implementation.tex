...
\end{equation}
Where the $K_P$ and $K_D$ coefficients are chosen to assure stability and reference tracking (HOW TO NOT EMPIRICALLY?)
...
\label{eq:gamma_dot}
\end{equation}
These equation can easily be obtained from the sum of forces in figure \ref{fig:flight_path_angle}. For the yaw rate comes that (SEE ANNEX)
\begin{equation}
...
\begin{figure}[!htb]
  ...
  \caption[Aircraft frames of reference]{$\gamma$ representation and frames of reference}
...
FINISH CONTROL WITH A METHOD TO GET Va GAMMA and PSI FROM X,Y,Z
...
\begin{equation}
\Delta = -K_P(\Omega-\Omega^d)-K_D(\ddot{\Omega}-\ddot{\Omega}^d)
\end{equation}
