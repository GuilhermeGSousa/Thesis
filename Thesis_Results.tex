...
In order to more accurately visualise the influence of the $\xi$ coefficient on the NLI controller for this same case, several simulations were made for values of $\xi$ from $0$ to $1$. For each simulation the mean and maximum error were computed, in order to obtain a plot of these errors for each of the three references. From figures \ref{fig:xi_max_error} and \ref{fig:xi_mean_error} can be concluded that the error becomes considerable $\xi<0.05$.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{Figures/Results/error_xi.png}
\caption[Maximum errors for $V_a$, $\gamma$ and $psi$]{Maximum errors for $V_a$, $\gamma$ and $psi$ for $\xi=[0,1]$}
\label{fig:xi_max_error}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{Figures/Results/mean_error_xi.png}
\caption[Mean errors for $V_a$, $\gamma$ and $psi$]{Mean errors for $V_a$, $\gamma$ and $psi$ for $\xi=[0,1]$}
